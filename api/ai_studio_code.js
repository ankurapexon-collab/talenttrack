// api/chat.js
// This file runs on Vercel's serverless platform.
// Set your GEMINI_API_KEY in Vercel's Environment Variables dashboard.
// Free API Key at: https://aistudio.google.com/apikey

const MODEL_CANDIDATES = [
  process.env.GEMINI_MODEL, // Manual override if configured
  'gemini-2.5-flash',
  'gemini-2.0-flash',
  'gemini-1.5-flash',
  'gemini-flash-latest'
...
async function callGemini(modelName, apiKey, system, prompt, includeThinkingConfig) {
  const defaultSystemInstruction = 
    "You are TalentTrack AI, an elite Talent Acquisition assistant. Always structure your responses cleanly, professionally, and comprehensively. Avoid raw markdown artifacts (like '#---' or '***'). Use clear numbered headers (e.g. '1. Technical Core'), bold sub-topics, and clean bullet lists. NEVER cut off or truncate answers midway — complete every response fully.";
  ...
  if (includeThinkingConfig) {
    // Turn off internal thinking tokens where supported so full budget is used for output
...
async function tryModel(modelName, apiKey, system, prompt) {
  ...
  // Fallback if thinkingConfig is not supported by the selected model version
...
export default async function handler(req, res) {
  ...
  if (!apiKey) {
    res.status(500).json({
      error: 'Server missing GEMINI_API_KEY. Add GEMINI_API_KEY in Vercel Environment Variables and redeploy.'
    });
    return;
  }
